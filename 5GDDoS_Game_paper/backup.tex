...
Several studies have been conducted to fight against DDoS attacks. In \cite{Alharbi}, the authors introduced NFV and edge computing architecture to design a two-stage DDoS mitigation network. \cite{Santoyo} presented an edged based detection scheme ensuring fast detection of DDoS attacks from IoT devices. The author in \cite{Mert} provided a similar paradigm that security function on a edge computing defend the DDoS attack. A multi-level DDoS mitigation framework was proposed by the authors of \cite{Yan} to mitigate the DDoS attack to IoT. In \cite{Ali}, the authors illustrated a three-tier intrusion detection and prevention system in SDN-IoT environment to assure security. The authors in \cite{Shen} implemented a Two-Phase DDoS detection system and showed that it can detect attacks with little overhead.

Some papers enhanced intrusion detection with different methods. \cite{Wang} and \cite{Xuanyuan} adopted the entropy-based methods to identify the DDoS attack and achieve a decent detection rate. \cite{10} developed a Deep Convolution Neural Network Q-learning model to prevent Mirai botnet. The authors of \cite{12} built dynamic deployment of classifiers based on reinforcement learning model to detect DDoS attacks.

Collaborative frameworks also provided a possible solution. \cite{13} devised a collaborative task offloading scheme to balance the task load between overloaded and underloaded cloudlets, which can ameliorate the overall performance without additional security frameworks. \cite{14} explored the cooperative defense strategy and provided an online cooperative defense framework. In \cite{17}, detecting and isolating malicious nodes for D2D communication was proposed as another solution to DDoS attacks.
...
In the simulation, we choose the cost function $C_m^v(\cdot)$ to be $C_m^v(x) = 100e^x$, which is convex and non-decreasing. For path loss model, we choose $g(d)=22log_{10}(d)+28+20log_{10}(f_c)$\cite{3gpp.36.814}, where $d$ is the distance between EUs and ASPs, and $f_c$ is the frequency band. Except (\ref{mal_ratio}), all the ratio of malicious users to normal users are set to 0.5. For every section except (\ref{num_cmp}), the number of end users are set to 100 per ASP. The key parameters are presented in \Cref{parameterlist}, and $U(a, b)$ represents the uniform distribution in $[a, b]$.
\begin{table}[!htb]
\caption{System Parameters}
\begin{tabular}{|c|c|c|c|c|}
\hline
 $\eta$ &$N_o$ & $p_i$ &$\chi_{ij}(MHz)$&$\lambda_i$ \\ \hline
 3500 & -100dBM\cite{chen2015efficient} & 23dBM\cite{3gpp.36.101}&$U(0.5, 0.6)$&$U(1, 3)$\cite{fan2018application}\cite{sun2016primal} \\ \hline
$|\mathcal{I}|$& $d_{ij}(ms)$ &$B_i$ &$f_m^v(GHz)$&$s_{ij}(kB)$ \\ \hline
5&$U(5, 100)$&20MHz&$U(0.6, 0.9)$&$U(4, 5)$\\ \hline
$\gamma_i$&distance&$\xi_i$&$\phi_i$ &$\Psi_{ij}$  \\ \hline
100&$U(50, 100)$&0.999&0.05&$U(4, 5)$\\ \hline
\end{tabular}
\label{parameterlist}
\end{table}
...
\label{MPO_opti}
As seen in \Cref{subfig-1:VMnum} and \Cref{subfig-2:MPOutil}, the boundary has been plot and shown in the figures. The ASP has the same optimal response in every interval. As the MPO price increases, the optimal responses by the ASPs changes from a decreasing, convex function ((\ref{eqn:asp_case2_utility_extreme}) and (\ref{eqn:asp_case3_utility_extreme})) to a constant. The MPO utility shown in \Cref{subfig-2:MPOutil} is a concave function in every interval, which is proven in the previous section.
...
In the paper, we proposed an optimal allocation rule to allocate the IPS VM to mitigate DDoS attacks. In addition, we solve the optimization problem of ASP and MPO by formulating the hierarchy relationship between the two users into a Stackelberg game.
