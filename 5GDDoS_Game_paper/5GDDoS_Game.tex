...
In the simulation, we choose the cost function $C_m^v(\cdot)$ to be $C_m^v(x) = 100e^x$, which is convex and non-decreasing. For path loss model, we choose $g(d)=22log_{10}(d)+28+20log_{10}(f_c)$, where $d$ is the distance between EUs and ASPs. Except \Cref{mal_ratio}, all the ratio of malicious users and normal users are set to be 0.5. The key parameters are presented in \Cref{parameterlist}.
\begin{table}[!]
...
Parameter &$\gamma_i$ & $d_{ij}$(ms) &$B_i$ &$f_m^v$(GHz)&$s_{ij}$(kB) \\ \hline
Value    &100 &uni(5,100)&20MHz&uni(0.6,0.9)&uni(4,5)\\ \hline
Parameter &$|\mathcal{I}|$&distance&$\xi_i$&$\phi_i$ &$\eta$  \\ \hline
Value    &50&uni(50,100)&0.999&0.05&3700\\ \hline
\end{tabular}
\label{parameterlist}
\end{table}
\begin{figure}[!]
    ...
    \\
    \subfloat[Malicious and Normal Ratio\label{subfig-1:ratio}]{%
      \includegraphics[width=0.25\textwidth]{5GDDoS_Game_utility_ratio.pdf}
    }
    \subfloat[Different Scheme of IPS VM Ratio\label{subfig-2:cmp}]{%
