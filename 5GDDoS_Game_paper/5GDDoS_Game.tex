...
In the simulation, we choose the cost function $C_m^v(\cdot)$ to be $C_m^v(x) = 100e^x$, which is convex and non-decreasing. For path loss model, we choose $g(d)=22log_{10}(d)+28+20log_{10}(f_c)$, where $d$ is the distance between EUs and ASPs, and $f_c$ is the carrier frequency. Except (\ref{mal_ratio}), all the ratio of malicious users to normal users are set to 0.5. The key parameters are presented in \Cref{parameterlist}.
\begin{table}[!htb]
