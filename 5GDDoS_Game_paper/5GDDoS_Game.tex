...
\end{equation}
If $Y_i^1(z_i^v|\Psi_m^v)$ is concave in feasible $z_i^v$, it is much easier to solve the optimization problem (\ref{eqn:asp_utility_opti}) with the objective being $Y_i^1(z_i^v|\Psi_m^v)$. We prove this properties in the following. 
\begin{lemma} \label{lemma:asp_case1_utility_concave}
$Y_i^1(z_i^v|\Psi_m^v)$ is concave in feasible $z_i^v$.
\end{lemma}
\begin{proof}
If $T_{i,1}^p(z_i^v)$ is convex in feasible $z_i^v$, $Y_i^1(z_i^v|\Psi_m^v)$ is concave in feasible $z_i^v$. We can verify the convexity of $T_{i,1}^p(z_i^v)$ by taking second derivative of $T_{i,1}^p(z_i^v)$ with respect to $z_i^v$
\begin{equation} \label{eqn:asp_case1_mm1_delay_second_deriv}
\frac{\partial^2 T_{i,1}^p(z_i^v)}{\partial (z_i^v)^2}=2[\big(z_i^v - G(\mu_i^v)\big)\mu_i^v - \lambda_i + H\big(G(\mu_i^v)\big)]^{-3}
\end{equation}
Since $(z_i^h)^* = G(\mu_i^v)$ satisfies the constraint (\ref{eqn:asp_utility_def_opti_const2}), (\ref{eqn:asp_case1_mm1_delay_second_deriv}) is always positive, which means $T_{i,1}^p(z_i^v)$ is convex in feasible $z_i^v$. Hence, $Y_i^1(z_i^v|\Psi_m^v)$ is concave in feasible $z_i^v$.
\end{proof}
We characterize the solution to the optimization problem (\ref{eqn:asp_utility_opti}) when the objective function (\ref{eqn:asp_utility_opti_obj}) equals $Y_i^1(z_i^v|\Psi_m^v)$ in the following theorem. Before stating the result, we define the feasible region of (\ref{eqn:asp_utility_opti_const1}) as $\Upsilon_i$ for ASP $i$, and (\ref{eqn:asp_utility_opti_const2}) as $\Upsilon_i^1$ when the $Y_i(z_i^v|\Psi_m^v)$ equals $Y_i^1(z_i^v|\Psi_m^v)$. When $\Upsilon_i \cap \Upsilon_i^1$ is an non-empty interval in $\mathbb{R}^+$, we express the left endpoint of $\Upsilon_i \cap \Upsilon_i^1$ as $\zeta_i^{1l}$ and the right endpoint of $\Upsilon_i \cap \Upsilon_i^1$ as $\zeta_i^{1u}$. 
\begin{theorem}\label{thm:asp_case1_optimal}
The solution to the optimization problem (\ref{eqn:asp_utility_opti}) when the objective function (\ref{eqn:asp_utility_opti_obj}) equals $Y_i^1(z_i^v|\Psi_m^v)$ is $(z_{i,1}^v(\Psi_m^v))^*$ 
\begin{subnumcases}{=\label{eqn:asp_case1_optimal_solution}}
  0 & if $\Upsilon_i \cap \Upsilon_i^1 = \emptyset$ \label{eqn:asp_case1_optimal_solution_individual_rationality} \\
  \zeta_i^{1l} & if $(z_{i,1}^{v,e}(\Psi_m^v))^* < \zeta_i^{1l}$ \label{eqn:asp_case1_optimal_solution_lower_boundary} \\ 
  \zeta_i^{1u} & if $(z_{i,1}^{v,e}(\Psi_m^v))^* > \zeta_i^{1u}$ \label{eqn:asp_case1_optimal_solution_upper_boundary} \\
  (z_{i,1}^{v,e}(\Psi_m^v))^* & if $(z_{i,1}^{v,e}(\Psi_m^v))^* \in [\zeta_i^{1l}, \zeta_i^{1u}]$ \label{eqn:asp_case1_optimal_solution_extreme} 
\end{subnumcases}
where
\begin{equation} \label{eqn:asp_case1_utility_extreme}
(z_{i,1}^{v,e}(\Psi_m^v))^* = \frac{1}{\mu_i^v}\sqrt{\frac{\sum_{j \in \mathrm{U}_i^n}\Psi_{ij}}{(b_i-a_i)\Psi_m^v}} + \frac{\lambda_i - H(G(\mu_i^v))}{\mu_i^v} + G(\mu_i^v)
\end{equation}
\end{theorem}
\begin{proof}
We begin by proving the feasbile region is a convex set. $\Upsilon_i$ is a convex set because $\Upsilon_i$ is a ray. Since we have proved the convexity of $Y_i^1(z_i^v|\Psi_m^v)$ in \Cref{lemma:asp_case1_utility_concave}, $\Upsilon_i^1$ is a convex set because any superlevel set of a concave function is a convex set. Since the intersection of two convex set is a convex set, $\Upsilon_i \cap \Upsilon_i^1$ is a convex set. When $\Upsilon_i \cap \Upsilon_i^1 = \emptyset$, which means the ASP $i$ cannot make the processign queue stable while has a positive utility no matter how many VMs ASP $i$ buys from the MPO. In this situation, the best action of ASP $i$ is not to buy any VM. By \Cref{lemma:asp_case1_utility_concave}, $Y_i^1(z_i^v|\Psi_m^v)$ is concave. When the extreme point of $Y_i^1(z_i^v|\Psi_m^v)$ is smaller than $\zeta_i^{1l}$, $\zeta_i^{1l}$ is the optimal point. Likewise, when the extreme point of $Y_i^1(z_i^v|\Psi_m^v)$ is larger than $\zeta_i^{1u}$, $\zeta_i^{1u}$ is the optimal point. When the extreme point of $Y_i^1(z_i^v|\Psi_m^v)$ falls in the interior of the feasible region, the extreme point is the optimal point. The extreme point of $Y_i^1(z_i^v|\Psi_m^v)$ can be solved when the first derivative of $Y_i^1(z_i^v|\Psi_m^v)$ with respect to $z_i^v$ equals zero.
\begin{equation} \label{eqn:asp_case1_utility_first_deriv}
\Big[\big(z_i^v - G(\mu_i^v)\big)\mu_i^v - \lambda_i + H\big(G(\mu_i^v)\big)\Big]^{2} = \frac{\sum_{j \in \mathrm{U}_i^n} \Psi_{ij}}{(b_i-a_i)\Psi_m^v}
\end{equation}
Starting from (\ref{eqn:asp_case1_utility_first_deriv}), it only takes simple algebraic operations to have the expression for the extreme point in (\ref{eqn:asp_case1_utility_extreme}). \qedhere
\end{proof}
From \Cref{thm:asp_case1_optimal}, we can obtain the MPO's price per VM $\Psi_m^i$ which makes the ASP $i$'s optimal response switch among (\ref{eqn:asp_case1_optimal_solution_lower_boundary}), (\ref{eqn:asp_case1_optimal_solution_upper_boundary}), and (\ref{eqn:asp_case1_optimal_solution_extreme}) by solving the $\Psi_m^v$ that satisfies $(z_{i,1}^{v,e}(\Psi_m^v))^* = \zeta_i^{1l}$ or $(z_{i,1}^{v,e}(\Psi_m^v))^* = \zeta_i^{1u}$. 
\begin{equation}
\begin{aligned}
&\Psi_{m,1}^{i,l} = \frac{\sum_{j \in \mathrm{U}_i^n}\Psi_{ij}}{\big(\mu_i^v(\zeta_i^{1u} - G(\mu_i^v)) - \lambda_i + H(G(\mu_i^v)\big)^2(b_i-a_i)} \\
&\Psi_{m,1}^{i,u} = \frac{\sum_{j \in \mathrm{U}_i^n}\Psi_{ij}}{\big(\mu_i^v(\zeta_i^{1l} - G(\mu_i^v)) - \lambda_i + H(G(\mu_i^v)\big)^2(b_i-a_i)}
\end{aligned}
\end{equation}
The sequence $\{\Psi_{m,1}^{i,l} , \Psi_{m,1}^{i,u}, \forall i \in \mathcal{I}\}$ partitions the space of the MPO's price per VM $\Psi_m^v$ into $2I + 1$ intervals.
\begin{equation}
0 \cup \mathbb{R}^{+} = \mathcal{M}_1^1 \cup \mathcal{M}_1^1 \cdots \cup \mathcal{M}_1^{2I+1}
\end{equation}
Within each interval $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$, we define four sets of ASP $i$ that has different optimal responses.
\begin{equation}
\begin{aligned}
&\Omega_1^{k,z} = \{i \in \mathcal{I}|(z_{i,1}^v(\Psi_m^v))^* = 0\} \\
&\Omega_1^{k,l} = \{i \in \mathcal{I}|(z_{i,1}^v(\Psi_m^v))^* = \zeta_i^{1l}\} \\
&\Omega_1^{k,u} = \{i \in \mathcal{I}|(z_{i,1}^v(\Psi_m^v))^* = \zeta_i^{1u}\} \\
&\Omega_1^{k,e} = \{i \in \mathcal{I}|(z_{i,1}^v(\Psi_m^v))^* = (z_{i,1}^{v,e}(\Psi_m^v))^*\} 
\end{aligned}
\end{equation}
These four sets form a partition of the all ASPs. That is, $\mathcal{I} = \Omega_1^{k,z} \cup \Omega_1^{k,l} \cup \Omega_1^{k,u} \cup \Omega_1^{k,e}, \, \forall k \in \{1, \cdots, 2I+1\}$. The total number of VMs bought by the ASPs within each interval $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$ can be expressed 
\begin{equation}\label{eqn:asp_case1_total_number_VM}
\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v))^* =  \sum_{i\in \Omega_1^{k,l}}\zeta_i^{1l} + \sum_{i\in \Omega_1^{k,u}}\zeta_i^{1u} + \sum_{i \in \Omega_1^{k,e}}(z_{i,1}^{v,e}(\Psi_m^v))^* 
\end{equation}
With the expression (\ref{eqn:asp_case1_total_number_VM}), the MPO's optimization problem in each interval $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$ is
\begin{maxi!}[2]
  {\Psi_m^v}
  {\Psi_m^v \cdot \sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v))^* - C_m^v\big(\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v))^*\big) \label{eqn:case1_mpo_utility_opti_obj}} 
  {\label{eqn:case1_mpo_utility_opti}}
  {}
  \addConstraint{\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v))^* \leq \mathcal{Q}_v}{\label{eqn:case1_mpo_utility_opti_const1}}
  \addConstraint{\Psi_m^v \in \mathcal{M}_1^k}{\label{eqn:case1_mpo_utility_opti_const2}}
\end{maxi!}
\begin{theorem}\label{thm:mpo_case1_convex_optimization}
In each interval $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$, the MPO's optimization problem (\ref{eqn:mpo_utility_opti}) is a convex optimization problem. The MPO's otpimal price per VM $\Psi_m^v$ is one of the optimal prices in each $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$ that gives the highest utility.
\end{theorem}
\begin{proof}
We start by proving that $\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v))^*$ is convex in $\Psi_m^v$ when $\Psi_m^v \in \mathcal{M}_1^k,\, \forall k \in \{1, \cdots, 2I+1\}$. In (\ref{eqn:asp_case1_total_number_VM}), only the second term pertains to $\Psi_m^v$. As the summation of convex function is also a convex function, it suffices to prove that $(z_{i,1}^v(\Psi_m^v))^*$ is convex in $\Psi_m^v$ for $i \in \Omega_1^{k,e}$. Taking the second derivative of (\ref{eqn:asp_case1_utility_extreme}) with respect to $\Psi_m^v$, we have 
\begin{equation}
\frac{\partial^2 (z_{i,1}^{v,e}(\Psi_m^v))^*}{\partial (\Psi_m^v)^2} = \frac{3}{4}(\Psi_m^v)^{-5/2}\frac{1}{\mu_i^v}\sqrt{\frac{\sum_{j \in \mathrm{U}_i^n}\Psi_{ij}}{(b_i-a_i)}} \geq 0
\end{equation}
Hence, $(z_{i,1}^v(\Psi_m^v))^*$ is convex in $\Psi_m^v \in \mathcal{M}_1^k,\, \forall k \in \{1, \cdots, 2I+1\}$. Since the sublevel set of a convex function is a convex set, the feasible region resulting from (\ref{eqn:case1_mpo_utility_opti_const1}) is a convex set for any value $\mathcal{Q}_v$. In addition, the feasible region of (\ref{eqn:case1_mpo_utility_opti_const2}) is also a convex set because the convex combination of two numbers in $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$ is still a number in $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$. Hence, the feasible region of the optimization problem (\ref{eqn:case1_mpo_utility_opti}) is the intersection of two convex set and thus is a convex set. To prove that the MPO's the optimization problem (\ref{eqn:case1_mpo_utility_opti}) is a convex optimization problem, what remains to do is to prove that the objective function (\ref{eqn:case1_mpo_utility_opti_obj}) is concave in $\Psi_m^v \in \mathcal{M}_1^k,\, \forall k \in \{1, \cdots, 2I+1\}$. Using (\ref{eqn:asp_case1_utility_extreme}) and (\ref{eqn:asp_case1_total_number_VM}), we can unfold the first term in (\ref{eqn:case1_mpo_utility_opti_obj}) as follows. 
\begin{align} \label{eqn:mpo_case1_utility_first_term}
&\Psi_m^v \cdot \sum_{i \in \Omega_1^{k,e}} (z_{i,1}^v(\Psi_m^v))^* \\
&= \Psi_m^v\sum_{i\in \Omega_1^{k,l}}\zeta_i^{1l} + \Psi_m^v\sum_{i\in \Omega_1^{k,u}}\zeta_i^{1u} + \sum_{i \in \Omega_1^{k,e}}\Psi_m^v(z_{i,1}^{v,e}(\Psi_m^v))^*  \nonumber\\
& = \Psi_m^v \Big[\sum_{i\in \Omega_1^{k,l}}\zeta_i^{1l} + \sum_{i\in \Omega_1^{k,u}}\zeta_i^{1u}+ \sum_{i \in \Omega_1^{k,e}} \big(\frac{\lambda_i - H(G(\mu_i^v))}{\mu_i^v} \nonumber \\
&+ G(\mu_i^v)\big) \Big]+ \sum_{i \in \Omega_1^{k,e}} \frac{1}{\mu_i^v}\sqrt{\frac{\Psi_m^v \cdot \sum_{j \in \mathrm{U}_i^n}\Psi_{ij}}{(b_i-a_i)}} \nonumber
\end{align}
The first term in the final expression (\ref{eqn:mpo_case1_utility_first_term}) is a linear function of $\Psi_m^v$ which is both convex and concave. The second term in the final expression (\ref{eqn:mpo_case1_utility_first_term}) is concave in $\Psi_m^v$. To see this, we check the concavity via the second order test. The second derivative of the second term is 
\begin{equation}
 -\frac{1}{4}(\Psi_m^v)^{-3/2}\sum_{i \in \Omega_1^{k,e}} \frac{1}{\mu_i^v}\sqrt{\frac{\sum_{j \in \mathrm{U}_i^n}\Psi_{ij}}{(b_i-a_i)}} \leq 0 
\end{equation}
Therefore, (\ref{eqn:mpo_case1_utility_first_term}) is concave in $\Psi_m^v$. The second term of (\ref{eqn:case1_mpo_utility_opti_obj}), $C_m^v\big(\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v))^*$, is convex in $\Psi_m^v$ as it is a composition of $C_m^v$ and $\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v)$ where $C_m^v$ is convex and non-decreasing, and $\sum_{i \in \mathcal{I}} (z_{i,1}^v(\Psi_m^v)$ is convex in $\Psi_m^v \in \mathcal{M}_1^k,\, \forall k \in \{1, \cdots, 2I+1\}$. Since a concave function minus a convex function yields a concave function, (\ref{eqn:case1_mpo_utility_opti_obj}) is concave in $\Psi_m^v \in \mathcal{M}_1^k,\, \forall k \in  \{1, \cdots, 2I+1\}$. In sum, the MPO's optimization problem (\ref{eqn:case1_mpo_utility_opti}) in each interval $\mathcal{M}_1^k, \forall k \in \{1, \cdots, 2I+1\}$ is a convex optimization problem. \qedhere
\end{proof}
...
\end{theorem}
\begin{proof}
We begin by proving the feasible region is a convex set. $\Upsilon_i$ is a convex set because $\Upsilon_i$ is a ray. Since we have proved the convexity of $Y_i^2(z_i^v|\Psi_m^v)$ in \Cref{lemma:asp_case2_utility_concave}, $\Upsilon_i^2$ is a convex set because any superlevel set of a concave function is a convex set. Since the intersection of two convex set is a convex set, $\Upsilon_i \cap \Upsilon_i^2$ is a convex set. When $\Upsilon_i \cap \Upsilon_i^2 = \emptyset$, which means the ASP $i$ cannot make the processign queue stable while has a positive utility no matter how many VMs ASP $i$ buys from the MPO. In this situation, the best action of ASP $i$ is not to buy any VM. By \Cref{lemma:asp_case2_utility_concave}, $Y_i^2(z_i^v|\Psi_m^v)$ is concave. When the extreme point of $Y_i^2(z_i^v|\Psi_m^v)$ is smaller than $\zeta_i^{2l}$, $\zeta_i^{2l}$ is the optimal point. Likewise, when the extreme point of $Y_i^2(z_i^v|\Psi_m^v)$ is larger than $\zeta_i^{2u}$, $\zeta_i^{2u}$ is the optimal point. When the extreme point of $Y_i^2(z_i^v|\Psi_m^v)$ falls in the interior of the feasible region, the extreme point is the optimal point. The extreme point of $Y_i^2(z_i^v|\Psi_m^v)$ can be solved when the first derivative of $Y_i^2(z_i^v|\Psi_m^v)$ with respect to $z_i^v$ equals zero.
...
\end{proof}
From \Cref{thm:asp_case2_optimal}, we can obtain the MPO's price per VM $\Psi_m^i$ which makes the ASP $i$'s optimal response switch among (\ref{eqn:asp_case2_optimal_solution_lower_boundary}), (\ref{eqn:asp_case2_optimal_solution_upper_boundary}), and (\ref{eqn:asp_case2_optimal_solution_extreme}) by solving the $\Psi_m^v$ that satisfies $(z_{i,2}^{v,e}(\Psi_m^v))^* = \zeta_i^{2l}$ or $(z_{i,2}^{v,e}(\Psi_m^v))^* = \zeta_i^{2u}$. 
\begin{equation} 
\begin{aligned}
...
\begin{maxi!}[2]
  {\Psi_m^v}
  {\Psi_m^v \cdot \sum_{i \in \mathcal{I}} (z_{i,2}^v(\Psi_m^v))^* - C_m^v\big(\sum_{i \in \mathcal{I}} (z_{i,2}^v(\Psi_m^v))^*\big) \label{eqn:case2_mpo_utility_opti_obj}} 
  {\label{eqn:case2_mpo_utility_opti}}
  {}
  \addConstraint{\sum_{i \in \mathcal{I}} (z_{i,2}^v(\Psi_m^v))^* \leq \mathcal{Q}_v}{\label{eqn:case2_mpo_utility_opti_const1}}
  \addConstraint{\Psi_m^v \in \mathcal{M}_2^k}{\label{eqn:case2_mpo_utility_opti_const2}}
\end{maxi!}
